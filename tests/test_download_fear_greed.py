...
class TestDownloadFearGreed:
    ...
    def test_run_writes_zarr_in_chronological_order(self, tmp_path: Path, requests_mock: Any) -> None:
    ...
    def test_lookback_days_forwarded_as_limit(self, tmp_path: Path, requests_mock: Any) -> None:
    ...
    def test_default_lookback_zero_means_all_history(self, tmp_path: Path, requests_mock: Any) -> None:
        requests_mock.get(self.URL, json=self._payload([(1700000000, 50)]))
        d = DownloadFearGreed(out_root=tmp_path, skip_if_fresh=False)
        d.run()
    ...
    def test_skip_if_fresh(self, tmp_path: Path, requests_mock: Any) -> None:
        ...
        d = DownloadFearGreed(out_root=tmp_path, skip_if_fresh=True, freshness_tolerance_hours=48)
        d.run()

        assert requests_mock.call_count == 0

    def test_empty_response_logs_warning(self, tmp_path: Path, requests_mock: Any, loguru_capture: List[str]) -> None:
        requests_mock.get(self.URL, json={"name": "FNG", "data": [], "metadata": {"error": "none"}})
        d = DownloadFearGreed(out_root=tmp_path, skip_if_fresh=False)
        d.run()
