...
// Generated by: aisland jenkins scaffold --project chainwind
// Source template: aisland/services/jenkins.py (JenkinsService.scaffold_pipelines)
// To regenerate:  aisland jenkins scaffold --project chainwind --force
// =========================================================================
pipeline {
    agent any
                        ...
                        always {
                            script {
                                if (fileExists('black-checkstyle.xml')) {
                                    recordIssues(
                                        id: 'black-chainwind',
                                        name: 'Black Formatting (Chainwind)',
...
" """
                        ...
                        always {
                            script {
                                if (fileExists('isort-checkstyle.xml')) {
                                    recordIssues(
                                        id: 'isort-chainwind',
                                        name: 'Isort Import Order (Chainwind)',
            ...
            steps {
                script {
                    if (fileExists('tests') && sh(script: "find tests -name '*.py' | grep -q .", returnStatus: true) == 0) {
                        sh "${VENV_BIN}/pytest tests --junitxml=test-report.xml --cov=chainwind --cov-report=xml:coverage.xml --cov-report=term"
                    } else {
                ...
                always {
                    script {
                        ...
                        if (fileExists('coverage.xml')) {
                            recordCoverage(
                                id: 'coverage-chainwind',
                                name: 'Chainwind Coverage',
    ...
    post {
        always {
            echo 'Chainwind Pipeline Complete.'
        }
        success {
            echo 'Chainwind is healthy.'
        }
        failure {
            echo 'Chainwind build failed. Please check linting or test failures.'
        }
    }
}
